...
export default function GdprPage(props) {
  ...
  const gdpr = t('gdpr:gdpr', { returnObjects: true }) as Object;
          ...
          {/* <Box>
            {Object.entries(gdpr).map(([key, value]) =>   
              <>
                {key === 'privacyPolicy' && 
                  <>
                    <Typography key={key} className={s.title} variant="h1" gutterBottom>
                      {t('title')}
                    </Typography>
                    <p>{value.lastUpdated}</p>
                    ...
                    {value.map(section => 
                      ...
                      {section.title === 'Interpretation and Definitions' &&
                        <>
                          ...
                          <p>{section.interpretation.description}</p>
                          <Typography variant="h3" gutterBottom>
                            {section.definitions.title}
                          </Typography>
                          <p>{section.definitions.description}</p>
                          <List dense={true} className={s.text}>
                            {Array.isArray(section.definitions.list) &&  
                              section.definitions.list.map((value, index) => 
                                <ListItem key={index}>
                                  <ListItemText
                                    primary={<Trans>{value}</Trans>}
                                  />
                                </ListItem>
                              )
                            }
...
<li>
<p><strong>Company</strong> (referred to as either &quot;the Company&quot;, &quot;We&quot;, &quot;Us&quot; or &quot;Our&quot; in this Agreement) refers to Artportable AB, Åsögatan 176, Stockholm, Sweden</p>
</li>
<li>
...
<li>
<p><strong>Website</strong> refers to Artportable, accessible from <a href="https://artportable.com" rel="external nofollow noopener" target="_blank">https://artportable.com</a></p>
</li>
<li>
...
export async function getStaticProps({ locale }) {
  return {
    props: {
      ...await serverSideTranslations(locale, ['common', 'footer', 'header', 'gdpr', 'support']),
    }
  }
}