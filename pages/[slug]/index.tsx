...
export default function slugPage({ category = null, productList = null, navBarItems = [] }: { category: Category, productList: ProductList, navBarItems: NavBarItem[] }) {
  const page = () => {
    ...
    if (productList)
      return <ProductListPage productList={productList} navBarItems={navBarItems} />
    return <></>
  }
  ...
  switch (pageType) {
    ...
    case 'productList':
      ...
      if (locale != productList.locale) {
        var newLocale = productList.localizations.find(productListLocale => productListLocale.locale == locale);
        if (newLocale == null) {
          return {
            notFound: true,
          }
...
export async function getStaticPaths() {
  ...
  if (categories)
    categoriesPaths = categories.map((category: Category) => ({
      params: { slug: category.slug }, locale: category.locale
    }))
  if (productlists)
    productListsPaths = productlists.map((productList) => ({
      params: { slug: productList.slug }, locale: productList.locale
    }))
