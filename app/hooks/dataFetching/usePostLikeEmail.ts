...
const usePostLikeEmail = () => {
  ...
  const likeEmail = useCallback(async (artwork, isLiked: boolean) => {
    ...
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/api/artworks/${artwork.Id}/like?mySocialId=${user.socialId.value}`, {
    ...
    const emailData = {...artwork, ...{
      ImageURL: imageURL,
      WebbURL: webbURL,
      UnsubscribeURL: unsubscribeURL,
      LikedByUser: formatUserName(user?.given_name?.value, user?.family_name?.value),
