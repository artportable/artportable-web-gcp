...
import { AggregatedActivity, connect, FeedAPIResponse, NotificationActivity, RealTimeMessage, StreamFeed } from 'getstream'
import { Badge, Box, Button, Collapse, debounce, Paper, Typography } from '@material-ui/core'
...
const NotificationIconButton = (props: NotificationIconButtonProps) => {
  ...
  const sortNotifications = (notificationActivities: NotificationActivity[]) => {
    if (notificationActivities.length > 0) {
      notificationCount.current = notificationActivities.length;
      setReadNotifications([...notificationActivities].filter(item => item.is_seen));
      setUnreadNotifications([...notificationActivities].filter(item => !item.is_seen))
    }
  }

  const handleNotificationResponse = (notificationsResponse: FeedAPIResponse) => {
  ...
  const recieveRealTimeUpdate = async (message: RealTimeMessage) => {
    ...
    if (userFeed.current) {
      if (message.new.length > 0) {
        notificationResponse = await userFeed.current.get({ limit: notificationCount.current + message.new.length });
      }
      else {
        notificationResponse = await userFeed.current.get({ limit: readNotifications.length + unreadNotifications.length });
      }
  ...
  const onPopoverClose = async () => {
    ...
    if (userFeed.current) {
      var unreadIds = unreadNotifications.map(notification => notification.id);
      // need to collect feed twice because first get with mark_seen updates feed after collection
      var notificationsResponse = await userFeed.current.get({ limit: 0, mark_read: unreadIds, mark_seen: unreadIds })
      notificationsResponse = await userFeed.current.get({ limit: notificationCount.current })
      handleNotificationResponse(notificationsResponse);
    }
  }
  ...
  return (
    <>
      <IconButton color="secondary" aria-label="account" onClick={(event) => setPopoverAnchorEl(event.currentTarget)}>
      ...
      <div ref={containerRef}>
        <Popover
          ...
          <Collapse className={s.container} in={open} timeout="auto" unmountOnExit >
